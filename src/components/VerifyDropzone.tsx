"use client";
/**
 * src/components/VerifyDropzone.tsx — God Mode "Optical Scanner"
 * ──────────────────────────────────────────────────────────────────
    ...
    >
      ...
      >
        ...
        <AnimatePresence mode="wait">
          {state.kind === 'IDLE' && (
            ...
            >
              <motion.div
                ...
                style={{
                  background: 'linear-gradient(180deg, rgba(0,212,170,0.12), rgba(0,212,170,0.02))',
                  border: '1px solid rgba(0,212,170,0.30)',
                  boxShadow: '0 8px 24px -6px rgba(0,212,170,0.35)',
                }}
                whileHover={reduceMotion ? {} : { scale: 1.06 }}
                transition={{ type: 'spring', stiffness: 380, damping: 22 }}
              >
                <FileArchive className="h-8 w-8 text-[#00D4AA]" strokeWidth={1.5} />
                <motion.div
                  aria-hidden
                  className="absolute -top-1.5 -right-1.5 flex h-6 w-6 items-center justify-center rounded-full bg-[#00D4AA]"
                  style={{ boxShadow: '0 0 14px rgba(0,212,170,0.8)' }}
                  animate={reduceMotion ? undefined : { scale: [1, 1.15, 1] }}
                  transition={{ duration: 2, repeat: Infinity, ease: 'easeInOut' }}
                >
                  <Upload className="h-3.5 w-3.5 text-[#07061A]" strokeWidth={2.5} />
                </motion.div>
              </motion.div>
              ...
              <p className="mt-1.5 text-[13.5px] leading-[1.65] text-[#A8A0D8]">
          ...
          {isBusy && <BusyView key="busy" state={state as Extract<VerifierState, { kind: 'UNZIPPING' | 'HASHING' | 'VERIFYING_SIGNATURE' }>} reduceMotion={!!reduceMotion} />}
      ...
      >
        {!reduceMotion && (
          <motion.span
            ...
            style={{ border: '1.5px solid rgba(0,212,170,0.5)' }}
        ...
        >
          <Scan className="h-8 w-8 text-[#00D4AA]" strokeWidth={1.5} />
        </motion.div>
      </div>

      <p className="mt-6 text-[10.5px] font-mono font-semibold uppercase tracking-[0.32em] text-[#00D4AA]">
        {state.kind.replace('_', ' ')}
      </p>
      <p className="mt-2 text-[17px] font-semibold tracking-tight text-white">
        {phase.label}
      </p>
      <p className="mt-1 font-mono text-[11px] text-[#A8A0D8]/80">{phase.sub}</p>

      {/* progress bar */}
      {phase.progress >= 0 && (
        <div
          className="mt-6 w-full max-w-[260px] overflow-hidden rounded-full"
          style={{ height: 3, background: 'rgba(255,255,255,0.08)' }}
        >
          <motion.div
            className="h-full rounded-full bg-[#00D4AA]"
            style={{ boxShadow: '0 0 10px rgba(0,212,170,0.8)' }}
            initial={{ width: '0%' }}
