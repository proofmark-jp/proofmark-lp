...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // ユーザーごとにメール送信（Resend APIを直接叩く）
    const sendPromises = certs.map(async (cert: any) => {
      ...
      await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          from: 'ProofMark <noreply@proofmark.jp>', // ※実際の送信元ドメインに合わせてください
          to: userEmail,
    ...
    return res.status(200).json({ message: `Successfully sent ${certs.length} upsell emails.` });
