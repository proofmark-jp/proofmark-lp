...
// ══════════════════════════════════════════════════════════════════
//  SSRクラッシュの完全遮断: @react-pdf/renderer を内包するモーダルを
...
export default function CertificateClient({ initialData, certId }: Props) {
    ...
    // ═══════════════════════════════════════════════════════════════
    //  State Heartbeat: サーバーから注入された initialData を初期値として
    //  ローカルステートに格納する。これにより TSA Auto-Sync Engine が
    //  このステートを setCert で叩き続けることができる。
    // ═══════════════════════════════════════════════════════════════
    const [cert, setCert] = useState<any>(initialData.cert);
    const [bundle, setBundle] = useState<ProcessBundlePublic | null>(initialData.bundle as ProcessBundlePublic | null);
    const [authorProfile, setAuthorProfile] = useState<any>(initialData.authorProfile);
    ...
    if (!cert) {
        return (
            <div className="min-h-screen bg-[#07061A] text-white flex flex-col justify-center items-center gap-6 print:bg-white print:text-black">
                ...
                <button onClick={() => router.push('/')} className="text-[#00D4AA] hover:text-white transition-colors border-b border-[#00D4AA] pb-1 print:hidden">トップに戻る</button>
            </div>
        );
    }
                        ...
                        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-8 border-b border-white/[0.08] pb-6 print:border-gray-300">
                            ...
                            <div className="flex flex-wrap items-center gap-3">
                                ...
                                <BreathingBadge
                                    ...
                                    icon={
                                        <>
                                            <img src="/proofmark-badge-founder.svg" alt="Founder" className="w-4 h-4 print:hidden" />
